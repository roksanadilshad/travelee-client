  ...
  useEffect(() => {
    ...
    if (tripDaysCount === 0) {
      ...
      starterActivities.forEach((act, index) => {
        dispatch(addActivity({ dayIndex: 0, activity: { ...act, id: Date.now() + index } }));
      });
    }
    ...
    <div className="min-h-screen mt-16 bg-[#F8FAFC] pb-40 lg:pb-32">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 px-4 lg:px-8 py-3 lg:py-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 lg:gap-4 overflow-hidden">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="lg:hidden text-gray-600 p-2"><FaBars size={18} /></button>
            <div className="bg-white p-2 rounded-xl text-[#0EA5A4] shadow-lg hidden sm:block"><FaSuitcaseRolling size={20} /></div>
            <input
          ...
          <div className="flex items-center gap-2 lg:gap-6 shrink-0">
            ...
            <InviteFriend tripId={tripId} tripTitle={trip.destination} currentUser={session?.user} socket={socket} onAutoSave={handleSaveTrip} />
            <button onClick={handleSaveTrip} className="bg-[#0EA5A4] text-white px-4 lg:px-6 py-2 rounded-full text-sm font-semibold transition-all">Save</button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row px-4 lg:px-8 py-6 lg:py-10 gap-8">
        <aside className={`${isSidebarOpen ? "block" : "hidden"} lg:block w-full lg:w-64`}>
          <h3 className="text-xs font-bold text-gray-400 uppercase mb-4 tracking-widest">Timeline</h3>
          <nav className="flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-2 no-scrollbar">
            ...
            <button onClick={() => dispatch(addDay())} className="flex-shrink-0 lg:w-full flex items-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed border-gray-200 text-gray-400 hover:text-[#0EA5A4] hover:border-[#0EA5A4] font-bold transition-all text-sm">+ Add Day</button>
          </nav>
        </aside>
        ...
        <main className="flex-1 w-full max-w-2xl mx-auto lg:mx-0">
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-sm text-gray-500">{trip.days[selectedDayIdx]?.activities?.length || 0} activities</p>
            </div>
            <button onClick={() => setActiveDay(selectedDayIdx)} className="bg-[#0EA5A4] text-white flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold shadow-lg transition-transform"><FaPlus size={12} /> Add</button>
          </div>
          ...
          <div className="relative space-y-4">
            ...
            {!trip.days[selectedDayIdx] || (trip.days[selectedDayIdx]?.activities || []).length === 0 ? (
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext 
                  items={(trip.days[selectedDayIdx]?.activities || []).map(a => a.id)} 
                  strategy={verticalListSortingStrategy}
                >
                  {(trip.days[selectedDayIdx]?.activities || []).map((act, index) => (
                    <SortableActivity key={`${act.id}-${index}`} act={act} onDelete={handleDelete} />
                  ))}
                </SortableContext>
              </DndContext>
            )}
          ...
          <div className="mt-16 pt-10 border-t border-gray-200">
            <h3 className="text-xl font-bold text-gray-800 mb-6">Trip Budget Summary</h3>
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[92%] sm:w-auto bg-gray-900 text-white px-6 py-4 rounded-2xl shadow-2xl z-50 flex items-center gap-6 border border-gray-700">
        <div className="flex gap-4">
          <div className="flex flex-col border-r border-gray-700 pr-4">
            <span className="text-[10px] text-gray-400 uppercase font-bold tracking-tight">Base Price</span>
            <span className="text-sm font-bold">${trip.basePrice || 0}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[10px] text-gray-400 uppercase font-bold tracking-tight">Estimated Total</span>
            <span className="text-xl font-black text-green-400">${totalCost}</span>
          </div>
        </div>
        <button onClick={handleSaveTrip} className="bg-[#0EA5A4] hover:bg-[#0c8a89] px-6 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg">Confirm Plan</button>
      </div>
    </div>
  );
}